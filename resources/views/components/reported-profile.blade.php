...
<div class="py-2 flex flex-wrap">
    <img src="{{ Storage::url($report->reportable->picture->url) }}" class="w-8 h-8 rounded-full">
    <div class="px-2 flex-1 flex flex-wrap">
        <div class="flex-1 flex justify-between">
            <a href="{{ route('profile', ['name' => $report->reportable->user->name]) }}" class="pr-2 flex gap-2">
                <h3 class="text-sm text-color-4 hover:text-color-2">{{ '@' . $report->reportable->user->name }}</h3>
            </a>
